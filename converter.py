...
class Converter:
    ...
    * Finally, generate docx with ``python-docx``.
    ...
    def __init__(self, pdf_file:str, password:str=None, text=None):
    ...
    def load_pages(self, start:int=0, end:int=None, pages:list=None):
        ...
        self.text.insert(ttk.END, self._color_output('[1/4] Opening document...').__str__() + "\n")
    ...
    def parse_document(self, **kwargs):
        ...
        self.text.insert(ttk.END, self._color_output('[2/4] Analyzing document...').__str__() + "\n")
        self.text.see(ttk.END)
    ...
    def parse_pages(self, **kwargs):
        ...
        self.text.insert(ttk.END, self._color_output('[3/4] Parsing pages...').__str__() + "\n")
        self.text.see(ttk.END)
        ...
        for i, page in enumerate(pages, start=1):
            ...
            self.text.insert(ttk.END, (i.__str__() + '/' + num_pages.__str__() + " page " + pid.__str__()) + "\n")
            self.text.see(ttk.END)
            try:
                page.parse(**kwargs)
                ...
                if not kwargs['debug'] and kwargs['ignore_page_error']:
                    ...
                    self.text.insert(ttk.END, ('Ignore page %d due to parsing page error'.format(pid, e)) + "\n")
                    self.text.see(ttk.END)
                else:
    ...
    def make_docx(self, docx_filename=None, **kwargs):
        ...
        self.text.insert(ttk.END, self._color_output('[4/4] Creating pages...') + "\n")
        self.text.see(ttk.END)
        ...
        for i, page in enumerate(parsed_pages, start=1):
            ...
            try:
                ...
                if not kwargs['debug'] and kwargs['ignore_page_error']:
                    ...
                    self.text.insert(ttk.END, 'Ignore page '+pid.__str__()+' due to making page error: ' + e.__str__() + "\n")
                    self.text.see(ttk.END)
                else:
    ...
    def convert(self, docx_filename:str=None, start:int=0, end:int=None, pages:list=None, **kwargs):
        ...
        self.text.insert(ttk.END, 'Start to convert ' + self.filename_pdf + "\n")
        self.text.see(ttk.END)
        ...
        self.text.insert(ttk.END, ('Terminated in {}'.format(perf_counter()-t0)) + "\n")
        self.text.see(ttk.END)
