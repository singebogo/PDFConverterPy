...
class Tableview(ttk.Frame):
    ...
    def _build_tableview_widget(self, coldata, rowdata, bootstyle):
        ...
        self._rightclickmenu_cell = TableCellRightClickMenu(self, self.text)
    ...
    def _header_double_leftclick(self, event):
        ...
        if region == "separator":
            ...
            try:
                if iid:
                    datas = self.get_row(iid=iid).values
                    if(datas and len(datas) > 0):
                        if(os.path.isfile(datas[0]) and os.path.exists(datas[0])):
                            self.pdfview.open_file(datas[0])
                            self.text.insert(ttk.END, "预览打开文件成功 "+ datas[0].__str__() + "\n")
                            self.text.see(ttk.END)
            except Exception as e:
                self.text.insert(ttk.END, "预览打开文件失败.... " + e.__str__() + "\n")
                self.text.see(ttk.END)
...
class TableCellRightClickMenu(tk.Menu):
    ...
    def __init__(self, master: Tableview, text=None):
        """
        Parameters:
        ...
        config = {
            ...
            "deletefiles": {
                "label": f'''✔  {MessageCatalog.translate("Delete selected rows && 同时删除文件")}''',
                "command": self.delete_selected_rows_phy,
            },
            "openbysysapp":{
                "label": f'''♡  {MessageCatalog.translate("使用系统默认软件打开")}''',
                "command": self.openBySysApp,
            },
        }
        ...
        self.add_command(cnf=config["deletefiles"])
        self.add_command(cnf=config["openbysysapp"])
    ...
    def delete_selected_rows_phy(self):
        """Delete the selected rows"""
        iids = self.view.selection()
        if len(iids) > 0:
            # setting to prev should be in master?
            # 获取要删除的数据
            files = [row.values[0] for row in self.master.get_rows(selected=True)]
                ...
                if files:
                    for file in files:
                        if file and os.path.exists(file):
                            try:
                                os.remove(file)
                                self.text.insert(ttk.END, "删除文件失成功.... " + file.__str__() + "\n")
                                self.text.see(ttk.END)
                            except Exception as e:
                                self.text.insert(ttk.END, "删除文件失败.... " + file.__str__() + "\n")
                                self.text.see(ttk.END)
            except Exception as e:
                self.text.insert(ttk.END, "删除文件失败.... " + files.__str__() + "\n")
                self.text.see(ttk.END)

    def openBySysApp(self):
        """open the selected rows"""
        iids = self.view.selection()
        if len(iids) > 0:
            # setting to prev should be in master?
            # 获取要删除的数据
            files = [row.values[0] for row in self.master.get_rows(selected=True)]
            if len(files) <= 0:
                self.text.insert(ttk.END, "至少选中一个文件 " + "\n")
                self.text.see(ttk.END)
                return
                        ...
                        if file and os.path.exists(file):
                            try:
                                ...
                                if platform.system() == "Windows":
                                    os.startfile(file)
                                    self.text.insert(ttk.END, "Windows 打开文件成功 " + file.__str__() + "\n")
                                    self.text.see(ttk.END)
                                elif platform.system() == "Darwin":
                                    subprocess.call(["open", file])
                                    self.text.insert(ttk.END, "Darwin 打开文件成功 " + file.__str__() + "\n")
                                    self.text.see(ttk.END)
                                else:
                                    self.text.insert(ttk.END, "打开文件失败： Unsupported operating system.... " + "\n")
                                    self.text.see(ttk.END)
                            except Exception as e:
                                self.text.insert(ttk.END, "系统打开文件失败.... " + file.__str__() + e.__str__() + "\n")
                                self.text.see(ttk.END)
            except Exception as e:
                self.text.insert(ttk.END, "系统打开文件失败.... " + files.__str__() + e.__str__() + "\n")
                self.text.see(ttk.END)
